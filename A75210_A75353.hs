...
prop_swap2 :: Edge Int -> Property
prop_swap2 g = property (swap(swap g) == g)
...
prop_isEmpty g  | Set.null(nodes g) = property (isEmpty g)
                | otherwise = property(not(isEmpty g))

--Prop isValid------------------
prop_isValid :: Graph Int -> Property
prop_isValid g | Set.null(edges g) = property (isValid g)
...
prop_isDAG d = isDAG d ==> forAll (elements $ elems $ nodes d) $ \v -> if (not(Set.null (adj d v)))
                                                                       then forAll (elements $ elems $ (adj d v)) $ \a -> v `notMember` reachable d (target a) 
                                                                       else property True

--Prop isForest----------------------
prop_isForest :: Property
prop_isForest = forAll (dag :: Gen(DAG Int)) $ \d -> isForest d ==> forAll (elements $ elems $ nodes d) $ \v -> length (adj d v) <= 1

--Prop isSubgraphOf-----------------
prop_isSubgraphOf :: Graph Int -> Property
prop_isSubgraphOf g | isEmpty g = property(isSubgraphOf Graph.empty g)
                    | otherwise = forAll (sub_graph g :: Gen(Graph Int)) $ \s -> (isSubgraphOf s g) && isSubsetOf (nodes s) (nodes g) && isSubsetOf (edges s) (edges g)

prop_isSubgraphOf2 :: Graph Int -> Property
prop_isSubgraphOf2 h = property(isSubgraphOf (Graph.empty) h)

sub_graph :: (Ord v, Arbitrary v) => Graph v -> Gen (Graph v)
sub_graph graph = do number <- choose(1,20)
                     ns <- listOf $ elements $ elems $ nodes graph
                     if (Prelude.null ns)
                     then return $ Graph {nodes = Set.empty, edges = Set.empty}
                     else ger_sub_edge (Graph {nodes = fromList ns, edges = Set.empty}) graph number

ger_sub_edge :: (Ord v, Arbitrary v) => Graph v -> Graph v -> Int -> Gen (Graph v)
ger_sub_edge graph graph_o 0 = do return graph
ger_sub_edge graph graph_o n = do f <- elements $ elems $ nodes graph
                                  s <- elements $ elems $ nodes graph
                                  if(isSubgraphOf (Graph (nodes graph) (insert  (Edge f s) (edges graph))) graph_o)
                                  then ger_sub_edge (Graph (nodes graph) (insert  (Edge f s) (edges graph))) graph_o (n-1)
                                  else ger_sub_edge graph graph_o (n-1)
...
prop_bft g | Set.null(nodes g) = label "Vazio" True
           | otherwise = forAll (elements $ elems $ nodes g) $ \v -> if not(Set.null(edges(bft g (singleton v))))    
                                                                     then forAll (elements $ elems $ edges(bft g (singleton v))) $ \a -> nodes(bft g (singleton v)) `isSubsetOf` nodes g && (singleton a `isSubsetOf` edges g || singleton(swap a) `isSubsetOf` edges g) 
                                                                     else property(nodes(bft g (singleton v)) `isSubsetOf` nodes g && edges(bft g (singleton v)) `isSubsetOf` edges g)
...
prop_isPathOf2 g | Set.null(nodes g) || Set.null(edges g) = label "Não existem caminhos" True
                 | otherwise = forAll (elements $ elems $ nodes g) $ \v -> forAll (elements $ elems $ (nodes g)) $ \v1 -> if ((path g v v1) /= Nothing)
                                                                                                                          then isPathOf (fromJust(path g v v1)) g
                                                                                                                          else True
...
prop_path g | Set.null(nodes g) || Set.null(edges g) = label "Não existem caminhos" True
            | otherwise = forAll (elements $ elems $ nodes g) $ \v -> forAll (elements $ elems $ (nodes g)) $ \v1 -> if ((path g v v1) /= Nothing)
                                                                                                                     then fromList(fromJust(path g v v1)) `isSubsetOf` edges g
                                                                                                                     else True

--Prop topo--------------------------
prop_topo :: Property
prop_topo = forAll (dag :: Gen(DAG Int)) $ \g -> prop_topoaux g (topo g)

prop_topoaux :: Graph Int -> [Set Int] -> Bool
prop_topoaux g [] = True
prop_topoaux g [a] = (all(\v -> Set.null(adj g v)) $ elems a)
prop_topoaux g (a:xs) = (all(\v -> Set.map target (adj g v) `isSubsetOf` unions xs) $ elems a) && prop_topoaux g xs